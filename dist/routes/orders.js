 ...
 * Create a new order
 */
router.post('/', auth_1.authenticateToken, (0, validation_1.validateRequest)(validateCreateOrder), async (req, res) => {
    try {
        if (!req.user) {
            res.status(401).json({
                success: false,
        ...
        const order = await orderService_1.OrderService.createOrder(orderData, dealerId);
        res.status(201).json({
            success: true,
            data: {
                order
...
router.get('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const result = await orderService_1.OrderService.getOrders(filters);
        res.json({
            success: true,
            data: result,
...
router.get('/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const order = await orderService_1.OrderService.getOrderById(orderId, userId);
        if (!order) {
            res.status(404).json({
                success: false,
                error: {
 ...
 */
router.put('/:id', auth_1.authenticateToken, (0, validation_1.validateRequest)(validateOrderUpdate), async (req, res) => {
    try {
        if (!req.user) {
            res.status(401).json({
                success: false,
        ...
        const order = await orderService_1.OrderService.updateOrder(orderId, updates, userId);
        if (!order) {
            res.status(404).json({
                success: false,
                error: {
...
router.post('/:id/cancel', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const order = await orderService_1.OrderService.cancelOrder(orderId, userId, reason);
        if (!order) {
            res.status(404).json({
                success: false,
                error: {
 ...
 * Get dealer's orders
 */
router.get('/dealer/my-orders', auth_1.authenticateToken, async (req, res) => {
    try {
        if (!req.user) {
            res.status(401).json({
                success: false,
        ...
        const result = await orderService_1.OrderService.getDealerOrders(dealerId, status, page, limit);
        res.json({
            success: true,
            data: result,
 ...
 * Get farmer's orders
 */
router.get('/farmer/my-orders', auth_1.authenticateToken, async (req, res) => {
    try {
        if (!req.user) {
            res.status(401).json({
                success: false,
        ...
        const page = req.query.page ? parseInt(req.query.page) : 1;
        const limit = req.query.limit ? parseInt(req.query.limit) : 20;
        const result = await orderService_1.OrderService.getFarmerOrders(farmerId, status, page, limit);
        res.json({
            success: true,
            data: result,
...
router.get('/statistics', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const userId = req.user._id.toString();
        const userType = req.query.userType || 'dealer';
        const statistics = await orderService_1.OrderService.getOrderStatistics(userId, userType);
        res.json({
            success: true,
            data: {
                statistics,
