...
class RoadmapService {
     ...
     */
    static async generateRoadmap(roadmapData, userId) {
        try {
            // Verify user exists
            ...
            const milestones = cropInfo.stages.map(stage => {
            ...
            // Create roadmap
            const roadmap = new FarmingRoadmap_1.FarmingRoadmap({
                userId,
                cropType,
                variety,
                location,
    ...
    static async getUserRoadmaps(userId) {
        try {
            ...
            const roadmaps = await FarmingRoadmap_1.FarmingRoadmap.find({
                userId,
                isActive: true
            })
    ...
    static async getRoadmapById(roadmapId, userId) {
        try {
            if (!mongoose_1.default.Types.ObjectId.isValid(roadmapId)) {
                throw new Error('Invalid roadmap ID');
            }
            const query = { _id: roadmapId, isActive: true };
            if (userId) {
                query.userId = userId;
            }
            const roadmap = await FarmingRoadmap_1.FarmingRoadmap.findOne(query)
                .populate('userId', 'name phone location.state location.district');
            return roadmap;
        }
        catch (error) {
     ...
     */
    static async updateMilestoneProgress(roadmapId, milestoneId, progressUpdate, userId) {
        try {
            if (!mongoose_1.default.Types.ObjectId.isValid(roadmapId)) {
                throw new Error('Invalid roadmap ID');
            }
            // Find roadmap and verify ownership
            const roadmap = await FarmingRoadmap_1.FarmingRoadmap.findOne({
                _id: roadmapId,
                userId,
                isActive: true
            });
     ...
     */
    static async getUpcomingMilestones(userId, days = 7) {
        try {
            const roadmaps = await FarmingRoadmap_1.FarmingRoadmap.find({
                userId,
                isActive: true,
     ...
     */
    static async getOverdueMilestones(userId) {
        try {
            const roadmaps = await FarmingRoadmap_1.FarmingRoadmap.find({
                userId,
                isActive: true,
     ...
     */
    static async getMRLRecommendations(location, cropType) {
        try {
            const query = {
            ...
            const roadmaps = await FarmingRoadmap_1.FarmingRoadmap.find(query)
                .sort({ createdAt: -1 })
                .limit(10)
                .lean();
            ...
            const uniqueRecommendations = allRecommendations.filter((rec, index, self) => index === self.findIndex(r => r.pesticide === rec.pesticide));
     ...
     */
    static async updateRoadmapSettings(roadmapId, settings, userId) {
        try {
            if (!mongoose_1.default.Types.ObjectId.isValid(roadmapId)) {
                throw new Error('Invalid roadmap ID');
            }
            const roadmap = await FarmingRoadmap_1.FarmingRoadmap.findOneAndUpdate({ _id: roadmapId, userId, isActive: true }, { $set: settings }, { new: true }).populate('userId', 'name phone location.state location.district');
            return roadmap;
        }
        catch (error) {
    ...
    static async deleteRoadmap(roadmapId, userId) {
        try {
            ...
            const result = await FarmingRoadmap_1.FarmingRoadmap.updateOne({ _id: roadmapId, userId, isActive: true }, { isActive: false });
     ...
     */
    static async getRoadmapStatistics(userId) {
        try {
            const [activeRoadmaps, completedRoadmaps] = await Promise.all([
                FarmingRoadmap_1.FarmingRoadmap.find({ userId, isActive: true, currentStage: { $ne: 'completed' } }),
                FarmingRoadmap_1.FarmingRoadmap.find({ userId, currentStage: 'completed' })
            ]);
