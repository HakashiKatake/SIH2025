...
class ChatbotService {
    ...
    async processTextQuery(query) {
        try {
            ...
            throw new errors_1.DatabaseError("Failed to process chat query", 500, "CHAT_QUERY_ERROR");
        }
    }
    /**
    ...
    async processVoiceQuery(query) {
        try {
            ...
            throw new errors_1.DatabaseError("Failed to process voice query", 500, "VOICE_QUERY_ERROR");
        }
    }
    /**
     * Get chat history for a user
     */
    async getChatHistory(userId, limit = 20, page = 1) {
        try {
            const skip = (page - 1) * limit;
            const chatHistory = await ChatMessage_1.ChatMessage.find({ userId })
                .sort({ createdAt: -1 })
                .limit(limit)
                .skip(skip)
                .lean();
            ...
            throw new errors_1.DatabaseError("Failed to retrieve chat history", 500, "CHAT_HISTORY_ERROR");
        }
    }
    /**
    ...
    generateMockResponse(message, language) {
        ...
        const advice = this.farmingAdviceDatabase[lang];
        const topics = this.relatedTopicsDatabase[lang];
        ...
        // Get related topics
        const relatedTopics = topics[category] || topics.general;
        return {
