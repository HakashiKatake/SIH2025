...
import { WeatherCache, WeatherAlert } from '../models/Weather';
import { FarmingRoadmap } from '../models/FarmingRoadmap';
...
export class DatabaseIndexingService {
   ...
   */
  static async createEssentialIndexes(): Promise<void> {
  ...
  private static async createUserIndexes(): Promise<void> {
    ...
    await collection.createIndex(
      { 'location.state': 1, 'location.district': 1, isActive: 1, createdAt: -1 },
      { name: 'user_location_active_created' }
    );
    
    await collection.createIndex(
      { preferredLanguage: 1, 'location.state': 1, isActive: 1 },
      { name: 'user_language_location_active' }
    );
    
    await collection.createIndex(
      { crops: 1, 'location.state': 1, isActive: 1 },
      { name: 'user_crops_location_active' }
    );
    
    await collection.createIndex(
      { farmSize: 1, 'location.state': 1, isActive: 1 },
      { name: 'user_farmsize_location_active' }
    );
   ...
   */
  private static async createProductIndexes(): Promise<void> {
    const collection = Product.collection;
    
    // Compound indexes for marketplace queries
    await collection.createIndex(
      { category: 1, 'location.state': 1, 'location.district': 1, isActive: 1, createdAt: -1 },
      { name: 'product_category_location_active_created' }
    );
    
    await collection.createIndex(
      { price: 1, category: 1, 'location.state': 1, isActive: 1 },
      { name: 'product_price_category_location_active' }
    );
    
    await collection.createIndex(
      { quantity: 1, unit: 1, category: 1, isActive: 1 },
      { name: 'product_quantity_unit_category_active' }
    );
    
    await collection.createIndex(
      { sellerId: 1, category: 1, isActive: 1, updatedAt: -1 },
      { name: 'product_seller_category_active_updated' }
    );
    
    // Geospatial index for location-based searches
    await collection.createIndex(
      { 'location.latitude': 1, 'location.longitude': 1, isActive: 1 },
      { name: 'product_geolocation_active' }
    );
   ...
   */
  private static async createCropAnalysisIndexes(): Promise<void> {
    const collection = CropAnalysis.collection;
    
    // Compound indexes for crop analysis queries
    await collection.createIndex(
      { userId: 1, 'analysisResult.healthStatus': 1, createdAt: -1 },
      { name: 'crop_user_health_created' }
    );
    
    await collection.createIndex(
      { cropType: 1, 'analysisResult.healthStatus': 1, createdAt: -1 },
      { name: 'crop_type_health_created' }
    );
    
    await collection.createIndex(
      { userId: 1, cropType: 1, createdAt: -1 },
      { name: 'crop_user_type_created' }
    );
    
    await collection.createIndex(
      { 'analysisResult.confidence': -1, createdAt: -1 },
      { name: 'crop_confidence_created' }
    );
   ...
   */
  private static async createChatMessageIndexes(): Promise<void> {
    const collection = ChatMessage.collection;
    
    // Compound indexes for chat queries
    await collection.createIndex(
      { userId: 1, messageType: 1, language: 1, createdAt: -1 },
      { name: 'chat_user_type_language_created' }
    );
    
    await collection.createIndex(
      { language: 1, confidence: -1, createdAt: -1 },
      { name: 'chat_language_confidence_created' }
    );
    
    await collection.createIndex(
      { relatedTopics: 1, language: 1, createdAt: -1 },
      { name: 'chat_topics_language_created' }
    );
   ...
   */
  private static async createWeatherIndexes(): Promise<void> {
    const weatherCacheCollection = WeatherCache.collection;
    const weatherAlertCollection = WeatherAlert.collection;
    
    // Weather cache indexes
    await weatherCacheCollection.createIndex(
      { locationKey: 1, expiresAt: 1 },
      { name: 'weather_cache_location_expires' }
    );
    
    await weatherCacheCollection.createIndex(
      { latitude: 1, longitude: 1, expiresAt: 1 },
      { name: 'weather_cache_coords_expires' }
    );
    
    // Weather alert indexes
    await weatherAlertCollection.createIndex(
      { userId: 1, isActive: 1, expiresAt: 1, createdAt: -1 },
      { name: 'weather_alert_user_active_expires_created' }
    );
    
    await weatherAlertCollection.createIndex(
      { alertType: 1, severity: 1, isActive: 1, createdAt: -1 },
      { name: 'weather_alert_type_severity_active_created' }
    );
   ...
   */
  private static async createRoadmapIndexes(): Promise<void> {
    const collection = FarmingRoadmap.collection;
    
    // Compound indexes for roadmap queries
    await collection.createIndex(
      { userId: 1, cropType: 1, isActive: 1, createdAt: -1 },
      { name: 'roadmap_user_crop_active_created' }
    );
    
    await collection.createIndex(
      { cropType: 1, 'location.state': 1, isActive: 1 },
      { name: 'roadmap_crop_location_active' }
    );
    
    await collection.createIndex(
      { userId: 1, currentStage: 1, isActive: 1 },
      { name: 'roadmap_user_stage_active' }
    );
    
    await collection.createIndex(
      { estimatedHarvest: 1, isActive: 1 },
      { name: 'roadmap_harvest_active' }
    );
   ...
   */
  static async dropCustomIndexes(): Promise<void> {
